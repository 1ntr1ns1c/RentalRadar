...
interface Inquiry {
  ...
  status: string;
  created_at: string;
  property?: { id: number; title: string; city: string; neighbourhood: string };
...
const Inquiries: React.FC = () => {
  ...
  const handleRespond = async (inquiryId: number) => {
    if (!responseText.trim() || !user?.token) return;
    try {
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      {inquiries.length === 0 ? (
        <div className="text-center py-8 text-gray-500">
          {user.role === 'landlord' ? 'No inquiries for your properties yet.' : "You haven't sent any inquiries yet."}
        </div>
      ) : (
          ...
          {inquiries.map((inquiry) => (
            <div key={inquiry.id} className="bg-white border rounded-lg p-6 shadow-sm">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-lg font-semibold">{inquiry.property?.title || `Property #${inquiry.listing_id}`}</h3>
                  <p className="text-gray-600">{inquiry.property?.city}, {inquiry.property?.neighbourhood}</p>
                  {user.role === 'landlord' && inquiry.tenant && (
                    <p className="text-sm text-gray-500">From: {inquiry.tenant.name} ({inquiry.tenant.email})</p>
                  )}
                </div>
                  ...
                  {respondingTo === inquiry.id ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex space-x-3">
                        <button onClick={() => handleRespond(inquiry.id)} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Send Response</button>
                        <button onClick={() => { setRespondingTo(null); setResponseText(''); }} className="bg-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-400">Cancel</button>
                      </div>
                    </div>
                  ) : (
                    <button onClick={() => setRespondingTo(inquiry.id)} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Respond</button>
                  )}
                </div>
              )}
              <div className="text-sm text-gray-500 mt-4">Sent on: {new Date(inquiry.created_at).toLocaleDateString()}</div>
            </div>
          ))}
        </div>
      )}
