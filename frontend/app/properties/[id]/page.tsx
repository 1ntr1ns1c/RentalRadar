...
export default function PropertyDetailPage() {
  ...
  const handleSendInquiry = async (e: React.FormEvent) => {
    ...
    try {
      await api.sendInquiry({ listing_id: property!.id, message: inquiryMessage }, user.token);
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-lg shadow-lg overflow-hidden mb-8">
          <ImageGallery images={property.images || []} alt={property.title} className="h-96" />
        </div>
        <div className="bg-white rounded-lg shadow-lg p-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <span className={`inline-flex px-3 py-1 text-sm font-semibold rounded-full ${property.is_available ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
          ...
          <div className="border-t pt-6">
            {user && user.role === 'tenant' ? (
              <div>
                ...
                {inquirySuccess && (
                  <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded mb-4">Inquiry sent successfully!</div>
                )}
              ...
              <div className="text-center">
                ...
                <button className="bg-gray-400 text-white py-3 px-6 rounded-lg font-semibold cursor-not-allowed">Your Property</button>
              </div>
            ) : (
              <div className="text-center">
                <p className="text-gray-600 mb-4">Please log in as a tenant to send inquiries.</p>
                <button className="bg-gray-400 text-white py-3 px-6 rounded-lg font-semibold cursor-not-allowed">Contact Landlord</button>
              </div>
            )}
          </div>
        </div>
