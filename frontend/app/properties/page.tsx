...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    ...
    if (filters.neighbourhood) filtered = filtered.filter(p => p.neighbourhood?.toLowerCase().includes(filters.neighbourhood.toLowerCase()));
  ...
  const clearFilters = () => setFilters({ city: '', neighbourhood: '', minPrice: '', maxPrice: '', bedrooms: '', bathrooms: '', propertyType: '' });
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <p className="text-gray-600">Find your perfect rental home from our curated selection</p>
        </div>
      </div>
          ...
          <div className="lg:w-3/4">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                {!loading && filteredProperties.length > 0 && (
                  <p className="text-gray-600 text-sm">Showing {filteredProperties.length} of {properties.length} properties</p>
                )}
              </div>
              {filteredProperties.length > 0 && (
                <div className="text-sm text-gray-500">Sort by: <span className="text-blue-600 font-medium">Relevance</span></div>
              )}
            </div>
              ...
              <div className="text-center py-12">
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-8">
                  ...
                  <p className="text-gray-600 mb-4">
                    {properties.length === 0 ? "No properties are currently available." : "No properties match your current filters."}
                  </p>
                  {properties.length > 0 && (
                    <button onClick={clearFilters} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Clear Filters</button>
                  )}
                </div>
              </div>
            ) : (
            ...
            {filteredProperties.length > 0 && filteredProperties.length < properties.length && (
              <div className="text-center mt-8">
                <button className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition-colors">Load More Properties</button>
              </div>
            )}
          </div>
        </div>
