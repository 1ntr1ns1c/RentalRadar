      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-16 max-w-7xl mx-auto">
        {features.map(({ title, description, icon, color }, i) => {
          ...
          return (
            <div
              key={i}
              className="p-8 bg-gray-50 rounded-xl shadow hover:shadow-lg transition-shadow"
            >
              <div
                className={`${colorSet.bg} ${colorSet.text} rounded-full w-16 h-16 flex items-center justify-center shadow-2xl mx-auto`}
              >
                <svg
                  className="w-8 h-8"
                  fill="none"
              ...
              <h3 className={`uppercase ${colorSet.text} font-semibold mt-6 mb-3 text-center`}>
                {title}
              </h3>
              ...
              <div className="mt-4 text-center">
                <a
                  href="/"
                  className={`${colorSet.text} ${colorSet.hover} font-medium flex items-center justify-center gap-1`}
                >
                  More about us
                  <svg
